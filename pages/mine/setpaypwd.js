...
import { getPhoneCode, getsetEdit, getSetPayment } from '../../network/authapi.js'
...
class setpaypwd extends React.Component {
    ...
    submit() {
        ...
        if (this.state.pwd == '') { return Toast.show('请输入交易密码') }
        if (this.state.re_pwd == '') { return Toast.show('请再次输入交易密码') }
        if (this.state.re_pwd != this.state.pwd) { return Toast.show('两次输入的密码不一致') }
        let { navigation } = this.props;
        var fromData = {};
        fromData['code'] = this.state.code;
        fromData['payment'] = this.state.pwd;
        fromData['payment_confirm'] = this.state.re_pwd;
    // console.log(fromData)
        getSetPayment(fromData, res => {
        // console.log(res)
            if (res.code == 1) {
        ...
        return (
            <View style={common.body}>
                ...
                <View style={[common.header]}>
                    ...
                    <View style={common.headerTitle}><Text style={common.headerTitleText}>修改交易密码</Text></View>
                </View>

                <View style={[css.main, { marginTop: 0, }]}>
                    {/* <View style={css.inputItem}>
                        <Text>注册账号</Text>
                        <TextInput style={{ width: width - 140 }} secureTextEntry={true} value={this.state.oldPwd} onChangeText={(text) => { this.setState({ oldPwd: text }) }} placeholder="请输入注册手机号" placeholderTextColor={'#D4D4D4'} />
                    </View> */}
                    <View style={css.inputItem}>
                        <Text>验证码</Text>
                    ...
                    <View style={css.inputItem}>
                        <Text>新密码</Text>
                        <TextInput style={{ width: width / 2 }} secureTextEntry={this.state.eye == 1 ? false : true} value={this.state.pwd} onChangeText={(text) => { this.setState({ pwd: text }) }} placeholder="请输入新的交易密码" placeholderTextColor={'#D4D4D4'} />
                        {
                            this.state.eye == 1 ?
