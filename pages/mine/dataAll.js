...
import { getLogin, getOrderLists, getOrderCancel, getStatisticsLists } from '../../network/authapi.js'
...
class addFriend extends React.Component {
        ...
        return (
            <View style={[common.whiteBg, common.padBottom, { flex: 1 }]}>
                ...
                <View style={[common.headerNoBorder]}>
                    ...
                    <View style={common.headerTitle}><Text style={common.headerTitleText}>数据统计</Text></View>
                </View>
                <View style={[common.navRowScrollView, { width: width, borderWidth: 0, height: height / 2 }]}>
                    <View style={[common.alignItemsE, { width: width - 20, }]}>
                        <TouchableOpacity onPress={() => this._showDatePicker()} style={[common.alignItemsCenter, { borderWidth: 1, borderColor: 'rgba(0,0,0,.1)', paddingRight: 20, borderRadius: 50, }]}>
                            <View style={[common.minYellowBtn, { marginLeft: 10, borderWidth: 0 }]}>
                                <Text>{date}</Text>
                            </View>
                            <Image source={require('../../image/down.png')} style={{ width: 15, height: 15, marginLeft: 10 }} />
                        </TouchableOpacity>
                    </View>
                    <View style={[common.navRowWrap, { justifyContent: 'space-around', width: width, marginTop: 15, height: 80, }]} horizontal={true}>
                        <View style={[common.columnCenter, { marginTop: 20 }]}>
                            <Text style={[common.navRowBig]}>{this.state.myData.today_achievement == null ? 0 : this.state.myData.today_achievement}</Text>
                            <Text style={[common.navRowBlock, { marginLeft: 0 }]}>当日数据</Text>
                        </View>
                        <View style={[common.columnCenter, { marginTop: 20 }]}>
                            <Text style={[common.navRowBig]}>{this.state.myData.month_achievement == null ? 0 : this.state.myData.month_achievement}</Text>
                            <Text style={[common.navRowBlock, { marginLeft: 0 }]}>本月数据</Text>
                        </View>
                    </View>
                </View>
    ...
    getStatisticsList() {
        ...
        getStatisticsLists(fromData, res => {
        // console.log(res)
            if (res.code == 1) {
                this.setState({
                    myData: res.data
