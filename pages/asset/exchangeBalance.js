...
import { getaddressIndex, getaddressDel, getCardIndex, getUserDetail, getWalletCash, getExchangeSetting, getWalletExchange } from '../../network/authapi.js'
...
class page extends React.Component {
        ...
        return (
            <View style={common.body}>
                ...
                <View style={[common.header]}>
                    ...
                    <TouchableOpacity onPress={() => { navigation.navigate('exchangeBalanceRecord'); }} style={common.headerRight}><Text style={common.headerRightText}>兑换记录</Text></TouchableOpacity>
                </View>
                <View style={[common.alignItemsCenter, { marginTop: 14, marginLeft: 20, }]}><Text style={{ color: '#A5A5A5' }}>当前可用积分：</Text><Text style={{ color: '#999999' }}></Text><Text style={{ color: '#999999', fontSize: 17, fontWeight: 'bold', marginTop: -3 }}>{this.state.all}</Text></View>
                <View style={[common.main, { paddingLeft: 20, paddingRight: 20, marginTop: 0 }]}>
                    <View style={[common.columnStart, css.pad]}>
                        <Text style={css.text}>兑换数量</Text>
                        <View style={common.alignItemsB}>
                            <View style={common.alignItemsCenter}>
                                ...
                                <TextInput
                                    ...
                                    value={this.state.inputValue} placeholder='请输入兑换数量' placeholderTextColor={'#D4D4D4'}
                                />
                            </View>
                            <TouchableOpacity onPress={this.allIn.bind(this)}><Text style={css.all}>全部兑换</Text></TouchableOpacity>
                        </View>
                    </View>
                    ...
                    <View style={[common.alignItemsCenter, { marginTop: 14 }]}><Text style={{ color: '#A5A5A5' }}>实际兑换：</Text><Text style={{ color: '#999999' }}></Text><Text style={{ color: '#999999', fontSize: 17, fontWeight: 'bold', marginTop: -3 }}>{this.state.get}</Text></View>
                    {/* </View> */}
    ...
    submit() {
        ...
        if (this.state.inputValue == '') { return Toast.show('请输入兑换数量'); }
    ...
    // console.log(fromData)
        getWalletExchange(fromData, res => {
        // console.log(res)
            if (res.code == 1) {
                Toast.show('提交成功');
                ...
                setTimeout(() => {
                    ...
                    that.getUserDetails()
                    navigation.navigate('exchangeBalanceRecord')
                }, 1000);
            } else if (res.code == -1) {
                Toast.show(res.msg);
                setTimeout(function () {
        ...
        // console.log(res)
            if (res.code == 1) {
                this.setState({
                    ...
                    all: res.data.userCoin.integral,
                })
            }
        })
    }
