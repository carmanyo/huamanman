      ...
      <View style={common.body}>
        ...
        <View style={[common.header]}>
          ...
          <View style={common.headerTitle}>
            <Text style={common.headerTitleText}>积分明细</Text>
          </View>
        </View>

        <View style={[common.tab, {marginTop: 20}]}>
          <TouchableOpacity onPress={this.tabClick.bind(this, 1)}>
            <Text
              style={[
                common.tabText,
                this.state.tab == 1 ? common.tabTextActive : null,
              ]}>
              收入
            </Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={this.tabClick.bind(this, 2)}>
            <Text
              style={[
                common.tabText,
                this.state.tab == 2 ? common.tabTextActive : null,
              ]}>
              支出
            </Text>
          </TouchableOpacity>
                ...
                <View style={css.recordBlock}>
                  <View style={common.alignItemsB}>
                    <View style={common.alignItemsCenter}>
                      <Text
                        style={[
                          this.state.tab == 1 ? common.red : common.green,
                          {marginRight: 10},
                        ]}>
                        {this.state.tab == 1 ? '收入' : '支出'}
                      </Text>
                      <Text>{item.typeName}</Text>
                    </View>
                    <Text
                      style={this.state.tab == 1 ? common.red : common.green}>
                      {this.state.tab == 1 ? '+' : '-'}￥{item.num}
                    </Text>
                  </View>
                  {item.desc != '' ? (
                    <Text style={css.time}>备注：{item.desc}</Text>
                  ) : null}
