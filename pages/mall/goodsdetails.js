...
class goodsdetails extends React.Component {
  ...
  render() {
    ...
    // console.log(imgArr)
    let maskBg = this.state.showMask == 1 ? <View style={common.mask} /> : null;
    let mask =
      this.state.showMask == 1 ? (
        <TouchableOpacity
          style={[common.mask, {opacity: 0}]}
      ...
      <View style={common.body}>
        ...
        <View style={[common.header]}>
          ...
          <View style={common.headerTitle}>
            <Text style={common.headerTitleText}>商品详情</Text>
          </View>
        </View>
        {this.state.myData.detail ? (
          <ScrollView
            style={[common.ScrollViewHasHeaderAndBottom]}>
            ...
            <View style={css.main}>
              ...
              <View style={css.goodsinfo}>
                ...
                <View style={css.priceWrap}>
                  <View style={css.nowPrice}>
                    ...
                    <Text style={css.priceStrong}>
                      {detail.goods_sku.goods_price}
                    </Text>
                  </View>
                  {/* <Text style={css.linePrice}>劵后价</Text> */}
                  {detail.goods_sku.goods_coupon != 0 ? (
                    <Text style={css.linePrice}>
                      劵可抵：{detail.goods_sku.goods_coupon}元
                    </Text>
                  ) : null}
                </View>
              ...
              <View style={css.details}>
                <View style={common.columnCenter}>
                  <Image
                    source={require('../../image/invite-2.png')}
                    style={css.inviteLogo}
                  />
        ...
        {maskBg}
        <Animated.View
          style={[common.modal, {transform: [{translateY: this.translateY}]}]}>
          {mask}
          {this.state.myData.detail ? (
            <View style={[common.tradeMain, {paddingBottom: 60}]}>
              <View style={common.goodsInfos}>
                ...
                <Image
                  source={{uri: this.state.myData.skuCoverImage}}
                  style={common.goodsImage}
                />
                <View>
                  <View style={[common.alignItemsCenter]}>
                    {/* <Text style={common.priceS}>劵后价：￥</Text> */}
                    <Text style={common.priceS}>￥</Text>
                    <Text style={common.priceB}>
                      {this.state.myData.goods_price}
                    </Text>
                    {detail.goods_sku.goods_coupon != 0 ? (
                      <Text style={css.linePrice}>
                        劵可抵：{detail.goods_sku.goods_coupon}元
                  ...
                  <View style={common.modalStockWrap}>
                    <Text style={common.modalStockNum}>
                      库存：{detail.goods_sku.stock_num}
                    </Text>
                  </View>
                </View>
              ...
              <View style={common.goodsAttr}>
                <View style={common.goodsAttrAcroll} scroll-y="true">
                  {this.state.myData.goodsMultiSpec &&
                    this.state.myData.goodsMultiSpec.spec_attr.map(
                      (item, index) => {
                        return (
                          <View style={common.groupItem} key={index}>
                            <Text style={common.tipsText}>
                              {item.group_name}
                            </Text>
                            <View
                              style={[
                ...
                {detail.is_online == 2 ? (
                  ...
                  <View style={common.alignItemsCenter}>
                    <TouchableOpacity onPress={this.jianNum.bind(this)}>
                      <Image
                        style={common.numImage}
                        source={require('../../image/jian.png')}
                      />
                    </TouchableOpacity>
                    <TextInput
                      onChangeText={value => this.setState({inputValue: value})}
                      value={this.state.inputValue}
                      style={common.numInput}
                      editable={false}
                    />
              ...
              {detail.goods_sku.stock_num == 0 ? (
                <TouchableOpacity
                  ...
                  <LinearGradient
                    ...
                    <Text style={common.linearBtnText}>暂无库存</Text>
                  </LinearGradient>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (Number(this.state.inputValue) < detail.start_num) {
                      Toast.show(
                        '该商品的购买数量必须大于等于' + detail.start_num,
                      );
                    } else {
  ...
  showModal() {
    ...
    Animated.timing(that.translateY, {
      toValue: 0,
      duration: 500,
  ...
  specItems(data, index) {
    // console.log(data)
    return data.map((item, indexs) => {
      return (
        // <TouchableOpacity onClick={() => this.onSwitchSpec(index,indexs)} key={indexs}></TouchableOpacity>
        <TouchableOpacity
          onPress={this.onSwitchSpec.bind(this, index, indexs)}
          key={indexs}
          style={{marginBottom: 10}}>
          {item.checked ? (
            <View>
              <Text style={[common.specValue, common.specValueActive]}>
                {item.spec_value}
              </Text>
            </View>
          ) : (
            <View>
              <Text style={[common.specValue]}>{item.spec_value}</Text>
            </View>
          )}
        </TouchableOpacity>
      );
      ...
      for (let j in goodsMultiSpec.spec_attr[i].spec_items) {
        if (attrIdx == i) {
          ...
          if (itemIdx == j) {
            ...
            goodsSpecArr[i] =
              goodsMultiSpec.spec_attr[i].spec_items[itemIdx].item_id;
          }
        }
      }
    }
  ...
  _updateSpecGoods() {
    ...
    if (typeof skuItem === 'object') {
      ...
      myData.line_price =
        skuItem.form.line_price +
        _this.state.myData.detail.goods_sku.goods_points;
      myData.stock_num = skuItem.form.stock_num;
      myData.skuCoverImage =
        skuItem.form.image_id > 0
          ? skuItem.form.image_path
          : _this.state.myData.detail.goods_image;
      _this.setState({
        myData,
      });
    }
  ...
  _initGoodsDetailData(data) {
    ...
    data.line_price =
      goodsDetail.goods_sku.line_price + goodsDetail.goods_sku.goods_points;
